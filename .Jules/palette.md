...
## 2025-06-13 - Dynamic Screen Reader Text on Toggle Buttons
**Learning:** For toggle buttons (like mobile menus), static screen reader text (e.g., `<span className="sr-only">Open main menu</span>`) is confusing when the menu is already open. Screen readers need to know the *next* action that will occur upon interaction, not the initial state.
**Action:** Always dynamically render the `sr-only` text and `title` attribute for toggle buttons to reflect the next state (e.g., `{isOpen ? 'Close menu' : 'Open menu'}`).
